...
class SQLiteEventStore:
    ...
    async def save_checkpoint(
        ...
        async with self._lock:
            ...
            try:
                self._begin()
                self._require_run_row(checkpoint.run_id)
                persisted_event = self._insert_event(event)
                ...
                self._rollback()
                raise self._raise_storage_error("save a checkpoint", exc) from exc
    ...
    async def get_latest_checkpoint(self, run_id: str) -> Checkpoint | None:
        ...
        async with self._lock:
            ...
            try:
                ...
                raise self._raise_storage_error("read a checkpoint", exc) from exc
