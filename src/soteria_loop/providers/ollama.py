...
:mod:`soteria_loop.config`. The ``AUTH_TOKEN``/``OPENAI_AUTH_TOKEN``
...
class OllamaConfig(BaseModel):
    ...
    def from_soteria_env(
        ...
        *,
        ...
        """Build config from the SOTERIA_OLLAMA_* environment variables."""

        model = environ.get("SOTERIA_OLLAMA_MODEL", "").strip() or fallback_model
        base_url = (environ.get("SOTERIA_OLLAMA_BASE_URL", "").strip() or _DEFAULT_BASE_URL).rstrip(
            "/"
        )
        api_key = environ.get("SOTERIA_OLLAMA_API_KEY", "").strip() or None
