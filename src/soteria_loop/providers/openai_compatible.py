...
Configuration comes from the SOTERIA_-prefixed environment documented in
:mod:`soteria_loop.config`. The legacy ``OPENAI_*`` variables used by the
live-benchmark harness remain supported through ``OpenAIConfig.from_env``
in :mod:`soteria_loop.providers.openai`.
"""
...
class OpenAICompatibleConfig(BaseModel):
    ...
    def from_soteria_env(
        ...
        *,
        ...
        """Build config from the SOTERIA_OPENAI_* environment variables."""

        api_key = environ.get("SOTERIA_OPENAI_API_KEY", "").strip()
        if not api_key:
            raise ValueError("SOTERIA_OPENAI_API_KEY is required when SOTERIA_PROVIDER=openai")

        model = environ.get("SOTERIA_OPENAI_MODEL", "").strip() or fallback_model
        base_url = (environ.get("SOTERIA_OPENAI_BASE_URL", "").strip() or _DEFAULT_BASE_URL).rstrip(
            "/"
        )
