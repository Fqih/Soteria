...
class AnthropicConfig(BaseModel):
    ...
    def from_soteria_env(
        ...
        *,
        ...
        """Build config from the SOTERIA_ANTHROPIC_* environment variables."""

        api_key = environ.get("SOTERIA_ANTHROPIC_API_KEY", "").strip()
        if not api_key:
            raise ValueError(
                "SOTERIA_ANTHROPIC_API_KEY is required when SOTERIA_PROVIDER=anthropic"
            )

        model = environ.get("SOTERIA_ANTHROPIC_MODEL", "").strip() or (
            fallback_model or _DEFAULT_MODEL
        )
        base_url = (
            environ.get("SOTERIA_ANTHROPIC_BASE_URL", "").strip() or _DEFAULT_BASE_URL
        ).rstrip("/")
