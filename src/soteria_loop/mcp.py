...
remote tool as a Soteria :class:`FunctionTool` so the runtime can
...
``mcp`` is an optional extra (``pip install soteria-loop[mcp]``); this
    ...
    async def _initialize(self) -> None:
        result = await self._request(
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {"name": "soteria-loop", "version": "0.1.0"},
            },
        )
...
# ---------------------------------------------------------------------------
# Adapter — wrap MCP tools as Soteria FunctionTools.
# ---------------------------------------------------------------------------
...
def mcp_tool(server: MCPServer, descriptor: JsonDict) -> PublicFunctionTool[Any]:
    """Wrap one MCP tool descriptor as a Soteria :class:`FunctionTool`."""
...
def mcp_tools(server: MCPServer) -> list[PublicFunctionTool[Any]]:
    """Wrap every tool exposed by ``server`` as a Soteria tool."""
