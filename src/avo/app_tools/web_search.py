...
_USER_AGENT = "Mozilla/5.0 (compatible; avo/1.0)"
...
class DuckDuckGoClient:
    ...
    async def search(self, query: str, *, max_results: int) -> list[dict[str, JsonValue]]:
        if _httpx is None:
            raise WebSearchError(
                "web_search requires httpx; install avo with the [providers] extra."
            )
...
async def _web_search(arguments: WebSearchArguments) -> dict[str, JsonValue]:
    if _httpx is None:
        raise WebSearchError("web_search requires httpx; install avo with the [providers] extra.")
