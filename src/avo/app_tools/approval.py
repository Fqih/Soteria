...
``APPROVAL_PENDING`` state (see ``src/avo/runtime.py`` and the
state-machine reference). This module produces a callable that consults
the ``AVO_TOOLS_REQUIRE_APPROVAL`` environment variable to decide
...
def _parse_env_list(environ: os._Environ[str] | dict[str, str]) -> set[str]:
    """Parse ``AVO_TOOLS_REQUIRE_APPROVAL`` into a normalized name set."""

    raw = environ.get("AVO_TOOLS_REQUIRE_APPROVAL", "")
    if not raw:
        return set()
    tokens: list[str] = []
...
def build_approval_callback(
    ...
    *,
    ...
    """Build an ``approval_callback`` that consults ``AVO_TOOLS_REQUIRE_APPROVAL``.
    ...
    Behavior:
        - Tool names listed in ``AVO_TOOLS_REQUIRE_APPROVAL`` → return
