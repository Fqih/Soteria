...
``AVO_USAGE_RATES`` (``AVO_USAGE_RATES_INPUT_PER_1K`` and
...
def _rates_from_env() -> tuple[Decimal, Decimal]:
    ...
    raw_in = os.environ.get("AVO_USAGE_RATES_INPUT_PER_1K", "").strip()
    raw_out = os.environ.get("AVO_USAGE_RATES_OUTPUT_PER_1K", "").strip()
    try:
