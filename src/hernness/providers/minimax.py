...
class MiniMaxConfig(BaseModel):
    ...
    def from_hernness_env(
        ...
        *,
        ...
        """Build config from the HERNNESS_MINIMAX_* environment variables.

        ``HERNNESS_MINIMAX_API_KEY`` is required and serves whichever style
        ``HERNNESS_MINIMAX_API_STYLE`` selects (default ``anthropic`` per the
        HERNNESS_ contract).
        """

        api_key = environ.get("HERNNESS_MINIMAX_API_KEY", "").strip()
        if not api_key:
            raise ValueError("HERNNESS_MINIMAX_API_KEY is required when HERNNESS_PROVIDER=minimax")

        model = environ.get("HERNNESS_MINIMAX_MODEL", "").strip() or fallback_model
        base_url = (
            environ.get("HERNNESS_MINIMAX_BASE_URL", "").strip() or "https://api.minimax.io"
        ).rstrip("/")
        api_style = environ.get("HERNNESS_MINIMAX_API_STYLE", "anthropic").strip().lower()
        if api_style not in ("openai", "anthropic"):
            raise ValueError(
                f"Unsupported HERNNESS_MINIMAX_API_STYLE {api_style!r}; "
