"""Interactive first-run setup wizard for ``hernness chat``.

When the REPL sees a missing or invalid ``HERNNESS_PROVIDER`` it asks the
...
def interactive_first_run_setup(
    ...
    *,
    ...
    Returns a dict of env-style variables (``HERNNESS_PROVIDER``,
    ``HERNNESS_<PROVIDER>_API_KEY``, ``HERNNESS_MODEL``, ...) ready to be
    ...
    try:
        stdout.write("\n")
        stdout.write("Hernness First-Time Setup\n")
        stdout.write("\n")
        ...
        env: dict[str, str] = {"HERNNESS_PROVIDER": provider_key}
        ...
        if spec["needs_api_key"]:
            ...
            env[f"HERNNESS_{uppercase_key}_API_KEY"] = api_key
        ...
        if provider_key == "minimax":
            ...
            env["HERNNESS_MINIMAX_API_STYLE"] = "anthropic" if style_choice == "1" else "openai"
            ...
            if base_url:
                env["HERNNESS_MINIMAX_BASE_URL"] = base_url
            ...
            if base_url:
                env["HERNNESS_OPENAI_BASE_URL"] = base_url
            ...
            if base_url:
                env["HERNNESS_OLLAMA_BASE_URL"] = base_url

        default_model = str(spec["default_model"])
        model = _prompt_optional(stdin, stdout, "Model", default=default_model).strip()
        env["HERNNESS_MODEL"] = model or default_model

        stdout.write(f"\nProvider configured: {provider_label} ({env['HERNNESS_MODEL']})\n")
        stdout.flush()
