...
class WebhookNotifier:
    ...
    def send(self, notification: Notification) -> None:
        if _httpx is None:
            raise NotifierError(
                "WebhookNotifier requires httpx; install hernness with the [providers] extra."
            )
        try:
...
def from_env() -> NotificationDispatcher:
    ...
    ``HERNNESS_NOTIFY_WEBHOOK`` — URL to POST to.
    ``HERNNESS_NOTIFY_DESKTOP`` — set to ``1`` to enable desktop notifications.
    """

    dispatcher = NotificationDispatcher()
    webhook = os.environ.get("HERNNESS_NOTIFY_WEBHOOK", "").strip()
    if webhook:
        dispatcher.register(WebhookNotifier(webhook))
    if os.environ.get("HERNNESS_NOTIFY_DESKTOP", "").strip() == "1":
