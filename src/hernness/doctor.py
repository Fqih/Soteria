"""``hernness doctor`` — verify HERNNESS_ setup without an HTTP call.

Reads ``os.environ`` (or an injected mapping) and reports which
HERNNESS_* variables are present, which are missing, and what the
resulting provider endpoint would be. Never sends a request to the
provider; use :mod:`hernness.chat` for an actual smoke test.

The doctor is intentionally synchronous and dependency-free so it works
in any shell where ``hernness`` is installed, even with no network
access.
"""
...
from hernness.config import _PROVIDER_NAMES, ConfigError, build_provider_from_env
...
_REQUIRED_BY_PROVIDER: dict[str, tuple[str, ...]] = {
    "ollama": ("HERNNESS_PROVIDER", "HERNNESS_MODEL"),
    "openai": ("HERNNESS_PROVIDER", "HERNNESS_MODEL", "HERNNESS_OPENAI_API_KEY"),
    "anthropic": ("HERNNESS_PROVIDER", "HERNNESS_MODEL", "HERNNESS_ANTHROPIC_API_KEY"),
    "minimax": ("HERNNESS_PROVIDER", "HERNNESS_MODEL", "HERNNESS_MINIMAX_API_KEY"),
}


@dataclass(frozen=True)
class DoctorReport:
    """Structured result of one ``hernness doctor`` invocation."""
...
def _provider_for(env: Mapping[str, str]) -> str | None:
    raw = env.get("HERNNESS_PROVIDER", "").strip().lower()
...
def _endpoint_for(env: Mapping[str, str], provider: str) -> tuple[str | None, str | None]:
    ...
    fallback_model = env.get("HERNNESS_MODEL", "x")
...
def run_doctor(environ: Mapping[str, str] | None = None) -> DoctorReport:
    ...
    model = env.get("HERNNESS_MODEL", "").strip() or None
    ...
    if provider is None:
        missing.append("HERNNESS_PROVIDER")
        ...
        api_key_var = f"HERNNESS_{provider.upper()}_API_KEY"
        has_api_key = bool(env.get(api_key_var, "").strip())

        base_url_key = f"HERNNESS_{provider.upper()}_BASE_URL"
    ...
    extra = tuple(sorted(k for k in env if k.startswith("HERNNESS_") and k not in set(missing)))
...
def render_report(report: DoctorReport, *, out: IO[str]) -> None:
    ...
    if report.ok:
        ...
        out.write(
            ...
            "the provider. Run `hernness chat` for an end-to-end smoke test.\n"
        )
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    When called from another entry point (e.g. the ``hernness`` CLI
    ...
    parser = argparse.ArgumentParser(
        prog="hernness doctor",
        description="Verify HERNNESS_ provider configuration without HTTP.",
    )
