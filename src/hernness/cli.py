...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="hernness",
        description="Inspect, resume, and chat with Hernness SQLite runs.",
    )
    parser.add_argument(
        "--database",
        "-d",
    ...
    commands.add_parser(
        "doctor",
        help="Verify HERNNESS_ provider configuration without making an HTTP call.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run the Hernness CLI and return a process exit status."""
