...
Mode is selected via ``HERNNESS_PERMISSION_MODE`` in the environment
(default ``default``).
"""
...
from hernness.runtime import ApprovalCallback
...
# ---------------------------------------------------------------------------
# Active-run + plan tracker (module-level; Hernness runs one agent at a time)
# ---------------------------------------------------------------------------
...
def permission_policy_from_env(environ: dict[str, str] | None = None) -> PermissionPolicy:
    """Build a :class:`PermissionPolicy` from the HERNNESS_PERMISSION_MODE env var."""

    env = environ if environ is not None else dict(os.environ)
    raw = env.get("HERNNESS_PERMISSION_MODE", "").strip().lower()
    if not raw:
    ...
    try:
        ...
        raise ValueError(f"HERNNESS_PERMISSION_MODE must be one of {allowed}; got {raw!r}") from exc
    extra_raw = env.get("HERNNESS_TOOLS_REQUIRE_APPROVAL", "").strip()
