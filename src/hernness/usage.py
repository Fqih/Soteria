...
``HERNNESS_USAGE_RATES`` (``HERNNESS_USAGE_RATES_INPUT_PER_1K`` and
``HERNNESS_USAGE_RATES_OUTPUT_PER_1K``).
...
def _rates_from_env() -> tuple[Decimal, Decimal]:
    ...
    raw_in = os.environ.get("HERNNESS_USAGE_RATES_INPUT_PER_1K", "").strip()
    raw_out = os.environ.get("HERNNESS_USAGE_RATES_OUTPUT_PER_1K", "").strip()
    try:
