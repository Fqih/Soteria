"""HERNNESS_-prefixed environment loader and provider factory.
...
``HERNNESS_PROVIDER`` / ``HERNNESS_MODEL`` dispatch table and validates the
...
the published reproduction commands stay stable. The new HERNNESS_ names
documented in this module are the official Hernness runtime API.
"""
...
class ConfigError(ValueError):
    """Raised when the HERNNESS_-prefixed environment is missing or invalid."""
...
def build_provider_from_env(
    ...
    *,
    ...
    """Build the configured ``ModelProvider`` from the HERNNESS_ environment.
    ...
    name = env.get("HERNNESS_PROVIDER", "").strip().lower()
    if name not in _PROVIDER_NAMES:
        allowed = ", ".join(_PROVIDER_NAMES)
        raise ConfigError(f"HERNNESS_PROVIDER must be one of {allowed!s}; got {name!r}")

    model = env.get("HERNNESS_MODEL", "").strip()
    if not model:
        raise ConfigError("HERNNESS_MODEL is required")
...
def apply_runtime_overrides(policy_kwargs: dict[str, Any], environ: Mapping[str, str]) -> None:
    """Apply HERNNESS_MAX_TOTAL_TOKENS / HERNNESS_MAX_RUNTIME_SECONDS /
    HERNNESS_REPEATED_ACTION_LIMIT.

    Mutates ``policy_kwargs`` in place; missing keys are no-ops so the loader
    never blocks startup.
    """

    tokens = environ.get("HERNNESS_MAX_TOTAL_TOKENS", "").strip()
    if tokens:
        try:
            policy_kwargs["max_total_tokens"] = int(tokens)
        except ValueError as exc:
            raise ConfigError(
                f"HERNNESS_MAX_TOTAL_TOKENS must be an integer; got {tokens!r}"
            ) from exc

    runtime = environ.get("HERNNESS_MAX_RUNTIME_SECONDS", "").strip()
    if runtime:
        try:
            policy_kwargs["max_runtime_seconds"] = float(runtime)
        except ValueError as exc:
            raise ConfigError(
                f"HERNNESS_MAX_RUNTIME_SECONDS must be a number; got {runtime!r}"
            ) from exc

    repeats = environ.get("HERNNESS_REPEATED_ACTION_LIMIT", "").strip()
    if repeats:
        try:
            policy_kwargs["repeated_action_limit"] = int(repeats)
        except ValueError as exc:
            raise ConfigError(
                f"HERNNESS_REPEATED_ACTION_LIMIT must be an integer; got {repeats!r}"
            ) from exc
...
def database_path_from_env(environ: Mapping[str, str] | None = None) -> str | None:
    ...
    env: Mapping[str, str] = os.environ if environ is None else environ
    value = env.get("HERNNESS_DATABASE_PATH", "").strip()
