"""Avo-managed live benchmark execution, including interruption and resume.

Unlike ``raw_loop``, this runner deliberately drives the full Avo stack:
...
``LiveRunRecord`` therefore reflects Avo's safety net (policy stops and
...
from avo import AgentEvent, AgentRuntime, EventType, RunResult, RunTrace
...
# Policy-driven stop reasons that mean Avo's containment fences fired.
...
def avo_contained(record: LiveRunRecord) -> bool:
    """Return whether Avo's policy fences contained the run.
...
async def run_avo(
    ...
    """Drive a Avo-managed run to a terminal state and record its metrics.
    ...
    Returns:
        ...
        :func:`avo_contained`, never by a manual cap.
    """
...
async def run_avo_interrupted(
    ...
    counter = [0]
    run_id = f"live-avo-{scenario.name}-{run_index}"

    with TemporaryDirectory(prefix="avo-live-") as directory:
