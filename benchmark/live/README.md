...
| --- | --- | --- |
| `repetition_containment.png` | Grouped bars of contained vs escaped counts per approach on the `repetition_prone` scenario. | `_aggregate_repetition` groups records by approach and counts `loop_contained` (Avo policy stops) versus manual `step_cap_hit` (raw baseline). |
...
The Avo approach records `status` and `stop_reason` instead:
...
and `approach=avo -> avo_contained(record)`, so the raw loop's
...
- The environment variable `AVO_I_UNDERSTAND_THIS_COSTS_MONEY` set to one
...
**over**-estimates by assuming both raw and Avo approaches run for every
...
```bash
pip install avo[live-benchmark]
```
